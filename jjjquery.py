...
@app.cell
def __(mo):
    minutes_slider = mo.ui.slider(start=5, stop=90, step=5, value=30, debounce=True, show_value=True)
    mo.md(
        f"""
...
def __(abc_radio, datetime, minutes_slider, mo, timedelta):
    ...
    recently_played = [
        ...
        for search_result in abc_radio.continuous_search(
            from_=now - timedelta(minutes=minutes_slider.value), to=now, station="triplej", limit=100
        )
